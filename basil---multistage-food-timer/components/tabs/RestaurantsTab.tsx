...
export const RestaurantsTab: React.FC<RestaurantsTabProps> = ({ country, state }) => {
  ...
  const [sourceUrls, setSourceUrls] = useState<{ uri: string, title: string }[]>([]);
  ...
  const fetchRestaurants = async () => {
    setIsLoading(true);
    const { restaurants: res, sourceUrls: sources } = await getVegRestaurants(country, state);
    ...
    <section className="space-y-24 animate-luxe">
      ...
      {isLoading ? <div className="text-center py-24 italic font-serif text-slate-300">Searching for the finest greens...</div> : (
        <>
          ...
          {/* Extract and list source URLs as required when using Google Search grounding */}
          {sourceUrls.length > 0 && (
            <div className="mt-16 pt-8 border-t border-[#e5e1da]">
              <h4 className="text-[10px] tracking-[0.3em] font-bold text-slate-400 uppercase mb-6 text-center">Data Sourced From</h4>
              <div className="flex flex-wrap justify-center gap-6">
                {sourceUrls.map((s, i) => (
                  <a key={i} href={s.uri} target="_blank" rel="noopener noreferrer" className="text-[9px] tracking-widest text-[#1a1a1a] hover:text-slate-500 transition-colors underline uppercase">
                    {s.title || 'Source'}
                  </a>
                ))}
              </div>
