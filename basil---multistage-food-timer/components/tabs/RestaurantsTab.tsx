...
export const RestaurantsTab: React.FC<RestaurantsTabProps> = ({ country, state, onApiError }) => {
  ...
  const fetchRestaurants = async () => {
    setIsLoading(true);
    // Wrap in try-catch to handle potential API errors from getVegRestaurants or generateRecipeImage
    try {
      const { restaurants: res } = await getVegRestaurants(country, state);
